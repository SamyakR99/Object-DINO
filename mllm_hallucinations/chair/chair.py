...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        for i in tqdm.trange(len(caps)):
            ...
            # add for precision
            num_correct_pred_objects += len(recall_gt_objects)
            num_pred_objects += len(node_words)
            ...
            if len(node_words) > 0:
                cap_dict['metrics']['Precision'] = len(recall_gt_objects) / len(node_words)

            if (cap_dict['metrics']['Recall'] + cap_dict['metrics']['Precision']) > 0:
                cap_dict['metrics']['F1'] = (
                    2 * cap_dict['metrics']['Recall'] * cap_dict['metrics']['Precision']
                ) / (cap_dict['metrics']['Recall'] + cap_dict['metrics']['Precision'])
        ...
        precision = num_correct_pred_objects / num_pred_objects if num_pred_objects > 0 else 0
