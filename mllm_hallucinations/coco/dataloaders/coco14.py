...
class CocoDetection(datasets.coco.CocoDetection):
    ...
    def __getitem__(self, index):
        ...
        img_pil = Image.open(os.path.join(self.root, self.data_split, path)).convert('RGB')
        
        # Create the transformed tensor for the model
        if self.transform is not None:
            img_tensor = self.transform(img_pil)
        else:
            img_tensor = img_pil # Fallback in case there's no transform

        # Return both the PIL image and the tensor
        return img_pil, img_tensor, target
